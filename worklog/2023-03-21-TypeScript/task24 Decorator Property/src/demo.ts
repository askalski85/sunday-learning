...
function auditable(target: object, key: string | symbol) {
    let val = target[key]; // save the initial value before doing any changes

    Object.defineProperty(target, key, {
        get: () => val,
        set: (newVal) => {
            console.log(`${key.toString()} changed: `, newVal) // Log every change to this value
            val = newVal // Do the actual modification
        },
        enumerable: true,
        configurable: true
    })
